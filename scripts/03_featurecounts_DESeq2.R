...
run_contrast <- function(groupA, groupB, tag) {
  if (all(c(groupA, groupB) %in% levels(colData(dds)$condition))) {
    ...
    res_list[[tag]] <<- r
    # Save with symbols (primary CSV outputs)
...
# ---------- ggplot MA plots with SYMBOL LABELS ----------
for (nm in names(res_list)) {
  # add symbols and choose label field once
  res_df <- add_symbols_to_results(res_list[[nm]], symbol_map)
  res_df$label <- ifelse(!is.na(res_df$symbol) & nzchar(res_df$symbol),
                         res_df$symbol, res_df$ensembl_gene_id)
  res_df$significant <- ifelse(res_df$padj < alpha_thresh, "Significant", "Not significant")
  ...
  p <- ggplot(res_df, aes(x = baseMean, y = log2FoldChange, color = significant)) +
    ...
    geom_text_repel(data = top_genes, aes(label = label), size = 3) +
    theme_minimal() +
    labs(
  ...
  ggsave(file.path(plot_dir, paste0("MA_", nm, "_ggplot_symbols.png")), p, width = 8, height = 6, dpi = 300)
}
